...
def save_extrinsic(sparse_path, extrinsics_w2c, img_files):
    images_bin_file = sparse_path / 'images.bin'
    images_txt_file = sparse_path / 'images.txt'
    images = {}
    
    image_suffix = Path(img_files[0]).suffix
...
def save_points3D(sparse_path, imgs, pts3d, confs, masks=None, use_masks=True, save_all_pts=False, save_txt_path=None, max_pts_num=150 * 10**10, mode='static'):
    allowed_modes = ['static', 'dynamic']
    if mode not in allowed_modes:
        raise ValueError(f"Invalid mode '{mode}'. Allowed values are: {allowed_modes}")
    
    points3D_bin_file = sparse_path / (mode+'_points3D.bin')
    points3D_txt_file = sparse_path / (mode+'_points3D.txt')
    points3D_ply_file = sparse_path / (mode+'_points3D.ply')

    # Convert inputs to numpy arrays
    imgs = to_numpy(imgs)
    pts3d = to_numpy(pts3d) #(14, 512, 272, 3)
    ...
    pts = pts.reshape(-1, 3) #maksed points (N, 3)
    ...
    storePly(points3D_ply_file, pts, col) #path, xyz, rgb
    if save_all_pts:
...
# Save images
def save_images_with_masks(outdir, n_views, imgs, masks, image_files):
    image_suffix = Path(image_files[0]).suffix
    images_path = os.path.join(outdir, 'images')
    masks_path = os.path.join(outdir, 'masks')
    images_path = Path(images_path)
    masks_path = Path(masks_path)
    images_path.mkdir(exist_ok=True, parents=True)
    masks_path.mkdir(exist_ok=True, parents=True)

    for i, (image, mask, name) in enumerate(zip(imgs, masks, image_files)):
        imgname = Path(name).stem
        image_save_path = images_path / f"{imgname}{image_suffix}"
        mask_save_path = masks_path / f"{imgname}{image_suffix}"

        # save masks
        mask_image = mask.detach().cpu().numpy() * 255
        cv2.imwrite(str(mask_save_path), mask_image.astype(np.uint8))
